...
parser.add_argument('-f', '--fin', metavar='DATE', type=str, default=str(datetime.date.today()), help='date rechercher la plus ancienne(format: AAAA-MM-JJ)')
...
def analyser_commande(s, v, d, f):
    if d == None:
        ...
        print(d)
    pass
    return ('titre={}: valeur={}, début=datetime.date({}), fin=datetime.date({})'.format(s, v, d.replace('-',', '), f.replace('-',', ')))
